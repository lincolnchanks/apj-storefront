...
// This component runs on a fixed delay, reads configuration (to get values for fixed delay
// and max age), calls the db cleanup API, and logs the report (the CartCleanupResponse, I
// assume). Basically this does a lot of heavy lifting the way I understand it.
...
public class CartCleanupJob {
    ...
    // This @Scheduled job is enabled by the @EnableScheduling annotation in WorkApplication.java.
    // It also uses cart.cleanup.fixed-delay-ms from application.properties, which in turn pulls
